// признаюсь, я не умею работать с json :(
// поэтому после безуспешных попыток разобраться с модулями, node.js и локальным сервером
// я сделала объект с данными прямо в js файле
let jsonData = [{
...
// создаю пустые массивы для нужных данных из jsonData
let fNameArr = [];
let lNameArr = [];
let aboutArr = [];
let eyeColorArr = [];

// в цикле заполняю массивы нужными данными
for (let item = 0; item < jsonData.length; item++) {
    fNameArr.push(jsonData[item].name.firstName);
    lNameArr.push(jsonData[item].name.lastName);
    aboutArr.push(jsonData[item].about);
